  ...
  methods: {
    ...
    getMenuTreeselect() {
      menuTreeselect().then(response => {
        this.menuOptions = this.handleTree(response.data,"id","parent","children","#");
      });
    },
    /** 查询部门树结构 */
    getDeptTreeselect() {
    ...
    getRoleMenuTreeselect(roleId) {
      roleMenuTreeselect(roleId).then(response => {
        ...
        for(let i in response.data){
          if(response.data[i].state.selected) arr.push(response.data[i].id)
        }
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            this.form.menuAll = this.getMenuAllCheckedKeys().join(",");
